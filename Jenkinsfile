...
// which are part of the Chita repo will allow modification to the Jenkinsfile.
...
for(int i = 0; i < targets.size(); i++) {
  ...
  tasks["${target}"] = {
    node {
      ...
      withEnv(env) {
        def builderImageName="chita-builder-${target}"
        ...
        builderImage.inside("-t") {
          ...
          sh "cp -ra ${pwd}/. /chita-src/"

          // restore cache
          def hasCache = false
          try {
            copyArtifacts(projectName: "chitapay-chita/${BRANCH_NAME}", optional: true, selector: lastSuccessful(), filter: "ci-cache-${target}.tar.gz")
          } catch (Exception e) {
          }
          if (fileExists("ci-cache-${target}.tar.gz")) {
            hasCache = true
            echo "Using cache from chitapay-chita/${BRANCH_NAME}"
          } else {
            try {
              copyArtifacts(projectName: 'chitapay-chita/develop', optional: true, selector: lastSuccessful(), filter: "ci-cache-${target}.tar.gz");
            } catch (Exception e) {
            }
            if (fileExists("ci-cache-${target}.tar.gz")) {
              hasCache = true
              echo "Using cache from chitapay-chita/develop"
            }
          }

          if (hasCache) {
            sh "cd /chita-src && tar xzf ${pwd}/ci-cache-${target}.tar.gz"
          } else {
            sh "mkdir -p /chita-src/ci-cache-${target}"
          }

          stage("${target}/depends") {
            sh 'cd /chita-src && ./ci/build_depends.sh'
          }
          stage("${target}/build") {
            sh 'cd /chita-src && ./ci/build_src.sh'
          }
          stage("${target}/test") {
            sh 'cd /chita-src && ./ci/test_unittests.sh'
          }
          stage("${target}/test") {
            sh 'cd /chita-src && ./ci/test_integrationtests.sh'
          }

          // archive cache and copy it into the jenkins workspace
          sh "cd /chita-src && tar czfv ci-cache-${target}.tar.gz ci-cache-${target} && cp ci-cache-${target}.tar.gz ${pwd}/"
        }
