...
# chita_hash
RUN git clone https://github.com/chitapay/chita_hash
RUN cd chita_hash && python3 setup.py install
...
RUN groupadd -g ${GROUP_ID} chita
RUN useradd -u ${USER_ID} -g chita -s /bin/bash -m -d /chita chita
...
RUN mkdir /chita-src && \
  ...
  chown $USER_ID:$GROUP_ID /chita-src && \
