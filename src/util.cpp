...
const char * const BITCOIN_CONF_FILENAME = "chita.conf";
const char * const BITCOIN_PID_FILENAME = "chitad.pid";
    ...
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL || *cacheCounter != logAcceptCategoryCacheCounter.load())
        {
            ...
            if (mapMultiArgs.count("-debug")) {
                ...
                // "chita" is a composite category enabling all Chita-related debug output
                if(ptrCategory->count(std::string("chita"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\Chita
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\Chita
    // Mac: ~/Library/Application Support/Chita
    // Unix: ~/.chita
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Chita";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Chita";
#else
    // Unix
    return pathRet / ".chita";
#endif
#endif
}
...
void ReadConfigFile(const std::string& confPath)
{
    ...
    if (!streamConfig.good()){
        // Create empty chita.conf if it does not excist
...

    {
        ...
        {
            // Don't overwrite existing settings so command line settings override chita.conf
